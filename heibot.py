        ...
        if len(line.split(":")) == 3:
            ...
            if message.find("`") != -1:
                ...
                if command.find("roll") != -1:
                    ...
                    if rang < (1.0 / slots):
                        slots = 6
                        s.send("PRIVMSG %s :*reloads the chambers*, there are %s untouched slots. You have a %s%% chance to die\n" % (irc['chan'], slots, "{0:.2f}".format(100.0 / slots)))
                        s.send("KICK %s %s You died! D:\n" % (irc['chan'], username))
                    else:
                        slots -= 1
                        s.send("PRIVMSG %s :*click*, there are %s untouched slots. You have a %s%% chance to die.\n" % (irc['chan'], slots, "{0:.2f}".format(100.0 / slots)))
